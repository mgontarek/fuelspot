...
- **`search-pipeline.ts`** — `createSearchPipeline(deps) → { run(route, getGpsState, i18n?) → SearchResult }` pure data pipeline: fetches POIs, ranks stops, returns results. Owns concurrency guard and hours-parser lifecycle. `createDefaultSearchPipeline(client?)` convenience factory wires production deps.
