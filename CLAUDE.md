...
- **`search-pipeline.ts`** — `createSearchPipeline(deps) → { run(route, getGpsState, i18n?) → SearchResult, runProximity(position, i18n?) → SearchResult }` pure data pipeline: fetches POIs, ranks stops, returns results. `run()` for route-based search, `runProximity()` for proximity-based 2-pin selection (nearest open + nearest unknown backup). Owns concurrency guard and hours-parser lifecycle. `createDefaultSearchPipeline(client?)` convenience factory wires production deps.
- **`upload.ts`** — DOM layer. `initUpload(geo?, client?, i18n?)` wires up file input, persists GPX to localStorage (`fuelspot-gpx`) via best-effort `trySaveRoute()` (catches `QuotaExceededError` and shows a non-blocking warning), displays route stats. Delegates search to `search-pipeline.ts` and maps results to UI calls. Supports two modes: **route mode** (auto-searches on first GPS fix after route load) and **proximity mode** (activates when no GPX loaded, auto-searches via `runProximity` on GPS fix, shows 2-stop card). `applyStaticTranslations(i18n)` updates `[data-i18n]` elements.
- **`result-card.ts`** — `initResultCard(container, i18n?) → ResultCardHandle` renders stops with status badge, distance, hours, and card payment info. `showStop(stop)` for single-stop route mode. `showStops(stops[])` for proximity mode: renders two sections (primary + backup) with labels when 2 stops, single stop when 1, empty message when 0. Handles loading, error, empty, and waiting-for-GPS states.
- **`route-map.ts`** — Leaflet map display with route visualization, POI pins, rider position, and highlighted #1 stop. `activate()` shows the map without a route (for proximity mode). `showRoute(route)` draws route polyline with start/finish markers.
...
- **`poi-fetcher.ts`** — Overpass API client with retry, cache, and POI parsing. `buildOverpassQuery(points)` for route-based search, `buildProximityQuery(lat, lng, radius)` for single-point proximity search. Exports `POI` type.
